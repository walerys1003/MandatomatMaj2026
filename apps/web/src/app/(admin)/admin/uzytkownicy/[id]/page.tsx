...
function statusBadge(status: string): string {
  if (status === 'paid' || status === 'succeeded') return 'bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300'
  if (status === 'pending' || status === 'paid_pending') return 'bg-amber-100 text-amber-700 dark:bg-amber-950 dark:text-amber-300'
  if (status === 'failed' || status === 'refunded') return 'bg-signal-100 text-signal-700 dark:bg-signal-950 dark:text-signal-300'
...
export default async function AdminUzytkownikDetailPage({
  ...
  const [profileRes, casesRes, paymentsRes, casesCountRes, paidCountRes] = await Promise.all([
    ...
    <div className="space-y-6">
        ...
        <div className="flex gap-2">
          <span
            className={`rounded-full px-3 py-1 text-xs font-medium ${
              profile.role === 'admin'
                ? 'bg-signal-100 text-signal-800 dark:bg-signal-950 dark:text-signal-300'
      ...
      <section className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <div className="rounded-lg border border-iron-200 bg-white p-4 dark:border-iron-700 dark:bg-iron-900">
          <p className="text-xs uppercase tracking-wide text-iron-500 dark:text-iron-400">Sprawy łącznie</p>
          <p className="mt-1 text-2xl font-bold tabular-nums text-iron-900 dark:text-iron-50">{totalCases}</p>
        </div>
        <div className="rounded-lg border border-iron-200 bg-white p-4 dark:border-iron-700 dark:bg-iron-900">
          <p className="text-xs uppercase tracking-wide text-iron-500 dark:text-iron-400">Sprawy opłacone</p>
          <p className="mt-1 text-2xl font-bold tabular-nums text-emerald-600 dark:text-emerald-400">
            {paidCases}
          </p>
        </div>
        <div className="rounded-lg border border-iron-200 bg-white p-4 dark:border-iron-700 dark:bg-iron-900">
          <p className="text-xs uppercase tracking-wide text-iron-500 dark:text-iron-400">Przychód</p>
          <p className="mt-1 text-2xl font-bold tabular-nums text-brand-700 dark:text-brand-300">
            {formatPln(fullRevenue)}
          </p>
        </div>
        <div className="rounded-lg border border-iron-200 bg-white p-4 dark:border-iron-700 dark:bg-iron-900">
          <p className="text-xs uppercase tracking-wide text-iron-500 dark:text-iron-400">Limit miesięczny</p>
      ...
      <section className="rounded-lg border border-iron-200 bg-white p-6 dark:border-iron-700 dark:bg-iron-900">
        <h2 className="mb-4 text-base font-semibold text-iron-900 dark:text-iron-50">Dane profilu</h2>
        <dl className="grid grid-cols-1 gap-3 text-sm sm:grid-cols-2 lg:grid-cols-3">
          <div>
            <dt className="text-xs uppercase tracking-wide text-iron-500 dark:text-iron-400">Email</dt>
            <dd className="text-iron-900 dark:text-iron-50">{profile.email}</dd>
          </div>
          <div>
            <dt className="text-xs uppercase tracking-wide text-iron-500 dark:text-iron-400">Imię i nazwisko</dt>
            <dd className="text-iron-900 dark:text-iron-50">{profile.full_name ?? '—'}</dd>
          </div>
          <div>
            <dt className="text-xs uppercase tracking-wide text-iron-500 dark:text-iron-400">Telefon</dt>
            <dd className="text-iron-900 dark:text-iron-50">{profile.phone ?? '—'}</dd>
          </div>
          <div className="sm:col-span-2 lg:col-span-3">
            <dt className="text-xs uppercase tracking-wide text-iron-500 dark:text-iron-400">Adres</dt>
          ...
          <div>
            <dt className="text-xs uppercase tracking-wide text-iron-500 dark:text-iron-400">Plan kończy się</dt>
            <dd className="text-iron-900 dark:text-iron-50">
              {profile.subscription_ends_at ? formatDate(profile.subscription_ends_at) : '—'}
            </dd>
          </div>
          <div>
            <dt className="text-xs uppercase tracking-wide text-iron-500 dark:text-iron-400">Kod referral</dt>
            <dd className="text-iron-900 dark:text-iron-50">
              <code className="text-xs">{profile.referral_code ?? '—'}</code>
            </dd>
          </div>
          <div>
            <dt className="text-xs uppercase tracking-wide text-iron-500 dark:text-iron-400">Polecony przez</dt>
            <dd className="text-iron-900 dark:text-iron-50">
              <code className="text-xs">{profile.referred_by ?? '—'}</code>
            </dd>
          </div>
          <div>
            <dt className="text-xs uppercase tracking-wide text-iron-500 dark:text-iron-400">Newsletter</dt>
            <dd className="text-iron-900 dark:text-iron-50">{profile.newsletter ? 'Tak' : 'Nie'}</dd>
          </div>
          <div>
            <dt className="text-xs uppercase tracking-wide text-iron-500 dark:text-iron-400">Email powiadomienia</dt>
            <dd className="text-iron-900 dark:text-iron-50">{profile.notification_email ? 'Tak' : 'Nie'}</dd>
          </div>
          <div>
            <dt className="text-xs uppercase tracking-wide text-iron-500 dark:text-iron-400">Utworzony</dt>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-iron-100 dark:divide-iron-800">
              {cases.length === 0 && (
                <tr>
                  <td colSpan={6} className="px-4 py-8 text-center text-iron-500 dark:text-iron-400">
                    Brak spraw.
                  </td>
                </tr>
              )}
              {cases.map((c) => (
                <tr key={c.id}>
                  ...
                  <td className="px-4 py-2 font-mono text-xs text-iron-600 dark:text-iron-400">{c.case_type}</td>
                  <td className="px-4 py-2">
                    <span className={`inline-block rounded-full px-2 py-0.5 text-xs ${statusBadge(c.status)}`}>
                      {c.status}
                    </span>
                  </td>
                  <td className="px-4 py-2 text-iron-700 dark:text-iron-300">{c.payment_status ?? '—'}</td>
                  <td className="px-4 py-2 text-right tabular-nums">
                    {c.amount_paid ? formatPln(c.amount_paid) : '—'}
                  </td>
                  <td className="px-4 py-2 text-iron-600 dark:text-iron-400">{formatDate(c.created_at)}</td>
                </tr>
              ))}
            </tbody>
          </table>
